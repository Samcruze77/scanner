...
// getLanding("/pdf-to-csv") always resolves (see utils/seo/landing.ts); the `!`
// documents that rather than adding a runtime check for a case that can't happen.
const landing = getLanding("/pdf-to-csv")!;
...
// CSV export lives in the scanner's own Export menu (/scan) -- this page is
...
export default function PdfToCsvPage() {
  return (
    <PageShell>
      <JsonLd
        data={graph(
          webPageSchema("/pdf-to-csv", landing.h1, landing.description),
          breadcrumbSchema("/pdf-to-csv", landing.crumbs),
        )}
      />
