...
await test("sign up: valid details create the account request and ask you to confirm your email", async () => {
  ...
  assert.equal(new URL(SUPABASE_URL + "/auth/v1" + c[0].path).searchParams.get("redirect_to"), BASE + "/", "sign-up sends emailRedirectTo = this site's home page, with the trailing slash the allow-list holds");
